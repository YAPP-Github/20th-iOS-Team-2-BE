...
public class FamilyCommandApi {
	...
	ResponseEntity<FamilyResponse.Create> createFamily(FamilyRequest.Create request) {
		return null;
	}

	@PatchMapping(value = "/family", consumes = APPLICATION_JSON_VALUE)
	ResponseEntity<Void> modifyFamilyInfo(FamilyRequest.Modify request) {
	...
	ResponseEntity<Void> createGreetingWithMessage(FamilyRequest.GreetWithMessage request) {
		return ResponseEntity.ok()
							 .build();
	}

	@PostMapping(value = "/family/greeting/emoji", consumes = APPLICATION_JSON_VALUE)
	ResponseEntity<Void> createGreetingWithEmoji(FamilyRequest.GreetWithEmoji request) {
		return ResponseEntity.ok()
							 .build();
	}
